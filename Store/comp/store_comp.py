...
collection_ref = db.collection("COMPANIES")  # Replace with your collection name

# Example: Retrieve documents from the collection
docs = collection_ref.get()


url = 'http://ec2-13-233-48-9.ap-south-1.compute.amazonaws.com:5000/sql/company/store'
headers = {
...
for doc in docs:
    print(f'{doc.id} => {doc.to_dict()["NAME"]}',end=" ")
    response = requests.post(url, headers=headers, json={"ID": doc.id, "NAME": doc.to_dict()["NAME"]})
    print(response.text)
